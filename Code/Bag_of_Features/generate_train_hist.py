...
# K-means clustering algo takes only 2 parameters which are number of clusters (k) and descrpitors list
...
def create_histogram(all_descs, kmeans):
    ...
    for key, value in all_descs.items():
        ...
        for desc in value:
            ...
            hist = np.array(
                np.bincount(raw_words, minlength=n_classes * clustering_factor)
            )
...
visual_words, kmeans = minibatchkmeans(
    n_classes * clustering_factor, all_train_descriptors
)
